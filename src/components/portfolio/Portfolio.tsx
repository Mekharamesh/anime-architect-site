...
function Contact() {
  ...
  const submit = (event: FormEvent) => { event.preventDefault(); if (status !== "idle") return; setStatus("loading"); requestAnimationFrame(() => { const spinner = buttonRef.current?.querySelector(".spinner"); anime({ targets: buttonRef.current, scaleX: [.98, 1], duration: 260, easing: EASE.quick }); anime({ targets: spinner, rotate: 360, loop: true, duration: 800, easing: "linear" }); }); window.setTimeout(() => { setStatus("sent"); requestAnimationFrame(() => { anime.remove(buttonRef.current?.querySelector(".spinner")); anime({ targets: checkRef.current, strokeDashoffset: [anime.setDashoffset, 0], duration: 600, easing: EASE.entrance }); anime({ targets: toastRef.current, opacity: [0, 1], translateY: [-20, 0], duration: 400, easing: EASE.entrance, complete: () => window.setTimeout(() => anime({ targets: toastRef.current, opacity: 0, translateY: -20, duration: 300, easing: EASE.quick }), 3000) }); }); }, 1300); };
